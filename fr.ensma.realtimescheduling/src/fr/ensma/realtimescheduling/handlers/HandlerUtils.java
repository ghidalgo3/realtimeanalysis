...
public class HandlerUtils {
	...
	static void showDialog(IWorkbenchWindow window, String message) {
		MessageDialog.openInformation(window.getShell(), "Analysis", message);
	}

	/**
	 * Displays a list of messages to the user or tells them that the system
	 * hasn't been validated.
	 * 
	 * @param results
	 *            Results of some analysis
	 * @param event
	 *            Used to get some UI information
	 * @throws ExecutionException
	 *             Eclipse non-sense
	 */
	static void holisticAnalysis(List<String> results, ExecutionEvent event)
			throws ExecutionException {
		IWorkbenchWindow window = HandlerUtil
				.getActiveWorkbenchWindowChecked(event);
		if (results == null) {
			HandlerUtils.showDialog(window,
