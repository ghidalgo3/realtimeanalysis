...
public class NetworkUtils {
	 ...
	 * algorithmic constraint on ports that says that the sum of 
	 * C_j / T_j for every flow V_j through a port h should sum to be 
	 * less than or equals to 1;
	 * @param ports All output ports
	 * @param flows All flows in the network
	 * @return A collection of port wrappers which know which flows flow through them
	 */
	public static Collection<PortWrapper> outputPortsToPortWrappers(Collection<Port> ports, List<Flow> flows) {
		return ports
				.stream()
				.map(p -> new PortWrapper(p, flows))
				.collect(Collectors.toSet());
	}
	...
	public static Module destinationForRoute(Route route) {
		...
		Stream<Port> destinationPortStream = virtualLink.getDestinations().stream().flatMap(mod -> mod.getModulePorts().stream());
		return (Module) destinationPortStream
				.filter(port -> allConnections.stream().anyMatch(conn -> conn.getA() == port || conn.getB() == port))
